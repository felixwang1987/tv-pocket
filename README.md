...
从 GitHub 仓库搜索发现相关公开项目，点播和直播搜索结果轮流取用仓库名额，读取 README 和候选配置，再解析配置子链接。结果按 URL 去重并保留发现来源。每天最多扫描 40 个仓库、检查 320 个候选，每仓库读取最多 8 个相关文件，每个文件限 3 MB、单次请求超时 12 秒；不是穷尽搜索整个 GitHub。
