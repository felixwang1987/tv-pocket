...
从 GitHub 仓库搜索发现相关公开项目，也读取 `sources.config.json` 指定的公开外部页面与直连配置。目前外部来源包括用户提供的 GYCK 线路合集和奇奇电话论坛帖子；论坛只解析正文中公开列出的地址，不执行网页脚本。点播和直播 GitHub 搜索结果轮流取用仓库名额，候选链接按 URL 去重并保留来源。每天最多扫描 40 个仓库、8 个指定外部页面、检查 360 个候选，其中给线路合集的下级配置预留 40 个检查名额；每仓库最多读取 8 个相关文件，配置文件限 3 MB、单次请求超时 12 秒。它不穷尽搜索整个互联网，外部页面也可能失效或改版。
