...
- **添加来源**：在网页填写公开直链或收集网页、普通/成人分类，点「去 GitHub 确认提交」。GitHub 会打开预填好的 Issue 表单；用仓库所有者账号确认提交后，工作流校验并加入每日检查。提交内容公开可见，勿填写账号、token 或私密链接。首次检查失败的直链会在来源目录显示原因；只有实播通过的线路才进固定合集。
