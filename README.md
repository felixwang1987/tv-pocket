...
从当前成功读取的单配置、多仓和线路合集递归展开，最多 3 层、额外读取 180 个配置。抽取不依赖外部插件、解析器、特殊请求头或账号参数的标准 JSON 点播接口，按 API 去重。每轮最多检测 100 个接口，每接口抽检至多 2 部节目的详情和直连视频画面；至少一部解码成功才发布。搜索单独测试，未确认搜索支持的站点仍可浏览。JAR/JS、网盘和其他特殊接口继续留在原目录，需影视仓实播，不混入已实测点播合集。
