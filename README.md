...
- **复制直播合集**：首页按钮会复制 `checked/live.m3u` 的线上地址，直接放入影视仓「直播配置」。只收录近 36 小时成功解码画面的抽检频道，按地址去重。未出画面的频道不会进入这份清单。
...
导出仅限 scripts/routes.py 中核对过的普通影视 API 主机名单，并要求抽检节目有明确的普通影视分类；未知来源不会因名称看起来正常就自动加入。分类检查针对抽样节目，不能过滤影视仓从原接口读取的整个片库。
